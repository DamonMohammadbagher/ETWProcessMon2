...
namespace VirtualMemAllocMon
{
    public partial class Form1 : Form
    {
        ...
        public struct SignatureII
        {
            public string bytes_str;
            public int prefix_suffix;
            public string bytes_str2;
            public int prefix_suffix2;
        }
        public struct DetectionDetails
        {
            public int size;
            public string Description;
        }
        public static List<VirtualMemAllocEvents> ETWVirtualMemAllocEvents = new List<VirtualMemAllocEvents>();
        public static List<DetectionDetails> db_Detection_Description = new List<DetectionDetails>();
        public static string ETW_VAx_Event_RealtimeChangedStrings = string.Empty;
        public static byte[] buf = new byte[208];
        public static string[] Flag_to_detection_VAx = new string[19];
        public static string[] Flag_to_detection_Bytes = new string[11];
        public static List<SignatureII> Flag_to_detection_BytesII = new List<SignatureII>();
        public static bool VaxFound, BytesFound = false;
        public static bool Is_EKKO = false;
        ...
        public static string DetectDesc = "";
        public static int foundindex_DetectDesc = -1;
        public static uint last_protection_mode;
        public delegate void _AsyncRunEkkoScanner(string pid , string startaddr , string etw_event_str);

        public static object[] SearchSection_v1(string input, string prefix, string suffix)
        {
            bool obj1 = false;
            object obj2 = 0;
            try
            {
                ...
                MatchCollection matches = Regex.Matches(input.ToUpper(), pattern.ToUpper());
                obj1 = matches.Count > 0;
                obj2 = matches;

                return new object[] { (object)obj1, (object)obj2 };
            }
            catch (Exception)
            {
        ...
        public static object[] _SearchVAxEvents(string input_to_search)
        {
            Is_EKKO = false;
            VaxFound = false;
            try
            {
                ...
                foreach (string item in Flag_to_detection_VAx)
                {
                    if (input_to_search.Contains(item))
                    {
                        ...
                        {
                            VaxFound = true;
                            int size_ekko_found = db_Detection_Description.FindIndex(detection => input_to_search.Contains(detection.size.ToString()));
                            if (db_Detection_Description[size_ekko_found].Description.Contains("EKKO SleepMask")) Is_EKKO = true; else Is_EKKO = false;
            ...
            {
                return new object[] { (object)false, (object)false };
            }
            
            return new object[] { (object)VaxFound, (object)Is_EKKO }; 
        }
        ...
        public static int _SearchBytes(string input_to_search)
        {
            ...
            try
            {
                ...
                foreach (string item in Flag_to_detection_Bytes)
                {
                        ...
                        /// string vv = "00000050   4D 5A 90 00 03 00 00 00  04 00 00 00 FF FF 00 00   MZ·········ÿÿ··";
                        object[] objsearched = SearchSection_v1(input_to_search.ToUpper(), "", item);
                        if ((bool)objsearched[0])
                        {
                            count++;
                            BytesFound = true;
                        }                  
                ...
                foreach (var item2 in Flag_to_detection_BytesII)
                {
                    try
                    {
                        ///string vv = "00000000   F9 F9 F9 B8 8C 08 AE 1C  FF E0 00 00 00 00 00 00   ùùù,?·r·ÿà······"
                        object[] objsearched = SearchSection_v1(input_to_search.ToUpper(), item2.bytes_str.ToUpper(), "");
                        if ((bool)objsearched[0])
                        {
                            count2 = count2 + 0.5;
                            BytesFound = true;

                            int start_byte_index = ((MatchCollection)objsearched[1])[0].Index;

                            bool objsearched1 = input_to_search.ToUpper().Substring(start_byte_index +
                                 item2.bytes_str.ToUpper().Length + 14).StartsWith(item2.bytes_str2.ToUpper());
            ...
            if (count2 > 0.5)
                count = count + Convert.ToInt32(count2);
        ...
        public  void _ShowDetailsBytes(string RealtimeChangedStrings)
        {
            ...
            {                          
                ...
                try
                {
                    Process p = Process.GetProcesses().ToList().Find(x => x.Id == (int)Convert.ToInt32(RealtimeChangedStrings.Split('(')[1].Split(')')[0]));
                    if (!p.HasExited)
                        _ProcessName = System.Diagnostics.Process.GetProcessById((int)Convert.ToInt32(RealtimeChangedStrings.Split('(')[1].Split(')')[0])).MainModule.FileName;
                }
                catch (Exception)
                {
                    _ProcessName = System.Diagnostics.Process.GetProcessById((int)Convert.ToInt32(RealtimeChangedStrings.Split('(')[1].Split(')')[0])).ProcessName;
                }
                ...
                if (result > 0)
                {
                    ...
                    {
                        foundindex_DetectDesc = -1;

                        foundindex_DetectDesc = db_Detection_Description.FindIndex(x =>
                        x.size == Convert.ToInt64(RealtimeChangedStrings.Split(':')[5]));

                        DetectDesc = "";

                        if (foundindex_DetectDesc != -1 && result == 1)
                            DetectDesc = db_Detection_Description[foundindex_DetectDesc].Description;
                        else
                            DetectDesc = "4D 5A 90 MZ bytes Detected";
                        

                        iList5 = new ListViewItem();
                        iList5.Name = DetectDesc + " In Process: " + _ProcessName + " With PID: " + __PID
                        ...
                        iList5.SubItems.Add(DetectDesc + " In Process: " + _ProcessName + " With PID: " + __PID
                             + "\nDetails: [TID: " + tid + "] with BaseAddress: "
                             + RealtimeChangedStrings.Split(':')[7] + "," + RealtimeChangedStrings.Split(':')[8]);
        ...
        public static object[] SearchSection(string input, string prefix, string suffix)
        {
            string pattern = $@"^([\w\.+-]|\s)*$ {prefix} ^([\w\.+-]|\s)*$ {suffix} ^([\w\.+-]|\s)*$";
            MatchCollection matches = Regex.Matches(input.ToUpper(), pattern.ToUpper());
            bool obj1 = matches.Count > 0;
            MatchCollection obj2 = matches;
            return new object[] { (object)obj1, (object)obj2 };
        }
        ...
        public void Program__Event_VirtualMemAlloc_etw_evt(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                object[] objfinds = _SearchVAxEvents(ETW_VAx_Event_RealtimeChangedStrings);


                /// normal detection , searching bytes in memory via signatures
                if ((bool)objfinds[0] && (bool)objfinds[1] == false)
                {

                    /// this will show Details of Detection for Jump Method and PE MZ bytes-Headers Detections 
                    _ShowDetailsBytes(ETW_VAx_Event_RealtimeChangedStrings);
                }

                /// abnormal detection [ekko method], waiting and Monitoring that process for 120secs to detecting RW 0x04 Protection Mode
                /// changes to RX 0x20 in loop time....
                /// if 10 times protection mode changed in that 120sec this will be flag of detection,
                /// and will be show that process as infected process by EKKO method
                if ((bool)objfinds[0] && (bool)objfinds[1] == true)
                {
                    try
                    {
                        /// this will show Details of Detection for EKKO Technique Which will run in background for 120secs
                        //MessageBox.Show(((IntPtr)Convert.ToUInt64(ETW_VAx_Event_RealtimeChangedStrings.Split(':')[4])).ToString("X8"));
                        string Ekko_targetAddress = ((IntPtr)Convert.ToUInt64(ETW_VAx_Event_RealtimeChangedStrings.Split(':')[4])).ToString("X8");

                        string __PID = ETW_VAx_Event_RealtimeChangedStrings.Split('(')[1].Split(')')[0];
                        Task.Run(() =>
                        {
                            System.Runtime.CompilerServices.TaskAwaiter<object[]> result =
                            Check_EKKO_Technique(Convert.ToInt32(__PID), Ekko_targetAddress, ETW_VAx_Event_RealtimeChangedStrings).GetAwaiter();
                        });
                    }
                    catch (Exception)
                    {
                ...
                ///old ver
                //if ((bool)_SearchVAxEvents(ETW_VAx_Event_RealtimeChangedStrings)[0])
                //{
        ...
        //public async void _RunAsync(string pid , string startaddr , string etw_evt_str)
        //{
        //    await Check_EKKO_Technique(Convert.ToInt32(pid), startaddr, etw_evt_str);
        //}
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            try
            {
                Flag_to_detection_BytesII.Add(new SignatureII { bytes_str = "b8", prefix_suffix = 0, bytes_str2 = "ff e0", prefix_suffix2 = 1 });
               // Flag_to_detection_BytesII.Add(new SignatureII { bytes_str = "f9 f9 f9 b8", prefix_suffix = 0, bytes_str2 = "ff e0", prefix_suffix2 = 1 });
                ...
                _VirtualMemAllocMon.WriteEntry("VirtualMemAllocMon v2.2 Started", EventLogEntryType.Information, 255);
            }
            catch (Exception)
            {
            ...
            Flag_to_detection_Bytes[2] = "4D 5A 41";
            Flag_to_detection_Bytes[3] = "4D 5A";
            /// CobaltStrike
            ...
            /// size: 4096 is for Jmp method detection
            Flag_to_detection_VAx[17] = ":4096:MEM_COMMIT, MEM_RESERVE:";
            db_Detection_Description.Add(new DetectionDetails { Description = "Jump Method b8 ?? ?? ?? ?? ff ef , Detected", size = 4096 });

            /// EKKO technique for havoc, rw section [size: 118784] 116kb allocation
            /// [1/16/2024 2:18:28 PM] PID:(8872) TID(2564) :47775744:118784:MEM_COMMIT, MEM_RESERVE:0x1d000:0x2dad000 [VirtualMemAlloc]
            /// EKKO via rtlcreatetimer for havoc
            /// [1/16/2024 2:35:41 PM] PID:(13112) TID(14228) :58916864:118784:MEM_COMMIT, MEM_RESERVE:0x1d000:0x384d000 [VirtualMemAlloc]
            Flag_to_detection_VAx[18] = ":118784:MEM_COMMIT, MEM_RESERVE:";
            db_Detection_Description.Add(new DetectionDetails { Description = "EKKO SleepMask via Havoc, ProtectionMode Chnaged From RW to RX , Detected", size = 118784 });


            // db_Detection_Description.Add(new DetectionDetails { Description = "Jump Method b8 ?? ?? ?? ?? ff ef , Detected", size = 4096 });
            //db_Detection_Description.Add(new DetectionDetails { Description = "EKKO SleepMask via Havoc, ProtectionMode Chnaged From RW to RX
            Thread.Sleep(250);
            try
            {
        ...
        private void Form1__VirtualMemAlloc_etw_evt_save_to_WindowsLog(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_items_Objects.ImageIndex == 2)
                    _VirtualMemAllocMon.WriteEntry(st.ToString(), EventLogEntryType.Warning, 1);
                else
                    _VirtualMemAllocMon.WriteEntry(st.ToString(), EventLogEntryType.Warning, 2);
        ...
        {
            MessageBox.Show(null, "VirtualMemAllocMon v2.2 [2.2.0.4]\nCode Published by Damon Mohammadbagher , Mar 2022", "About ETW VirtualMemAllocMon v2.2",
                MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        public async Task<object[]> Check_EKKO_Technique(int pid , string startaddress, string etw_event_string )
        {
            await Task.Run(() =>
            {
                int RW_count = 0;
                ...
                var _Delay = Task.Delay(TimeSpan.FromSeconds(120));
                do
                {

                    IntPtr ph = OpenProcess(ProcessAccessFlags.All, false, pid);

                    IntPtr address = new IntPtr(long.Parse(startaddress, System.Globalization.NumberStyles.HexNumber));
                    MEMORY_BASIC_INFORMATION info;

                    int result = VirtualQueryEx(ph, address, out info, Marshal.SizeOf<MEMORY_BASIC_INFORMATION>());

                    if (result != 0)
                    {
                        protection = info.Protect;

                        if (last_protection_mode != info.Protect)
                        {
                            //Console.WriteLine($"{DateTime.Now.ToString()} Protection mode for address 0x" +
                            //    $"{((IntPtr)Convert.ToUInt64(address.ToString())).ToString("X8")} : {protection.ToString("X2")}");

                            last_protection_mode = protection;

                            ///        EXECUTE = 0x10,     ==> medium risk for detection
                            //         EXECUTE_READ = 0x20,     ==> medium/high risk
                            ///        EXECUTE_READWRITE = 0x40,    ==> high risk
                            ///        EXECUTE_WRITECOPY = 0x80,
                            ///        NOACCESS = 0x01,   ==> low/medium risk
                            ///        READONLY = 0x02,
                            //         READWRITE = 0x04,     ==> low risk
                            ///        WRITECOPY = 0x08

                            if (protection.ToString("X2") == "04") RW_count++;
                            if (protection.ToString("X2") == "20") RX_count++;
                        }
                    }
                    else
                    {
                    ...
                    if (RW_count == 11 && RX_count == 11)
                    {

                        string _ProcessName = "";
                        try
                        {
                            Process p = Process.GetProcesses().ToList().Find(x => x.Id == (int)Convert.ToInt32(etw_event_string.Split('(')[1].Split(')')[0]));
                            if (!p.HasExited)
                                _ProcessName = System.Diagnostics.Process.GetProcessById((int)Convert.ToInt32(etw_event_string.Split('(')[1].Split(')')[0])).MainModule.FileName;
                        }
                        catch (Exception)
                        {
                            _ProcessName = System.Diagnostics.Process.GetProcessById((int)Convert.ToInt32(etw_event_string.Split('(')[1].Split(')')[0])).ProcessName;
                        }

                        string __PID = etw_event_string.Split('(')[1].Split(')')[0];
                        string tid = etw_event_string.Split('(')[2].Split(')')[0];

                        IntPtr phx = OpenProcess(ProcessAccessFlags.All, false, Convert.ToInt32(etw_event_string.Split('(')[1].Split(')')[0]));

                        temp = 0;
                        NtReadVirtualMemory(phx, ((IntPtr)Convert.ToUInt64(etw_event_string.Split(':')[4])), buf, (uint)buf.Length, ref temp);

                        dumpmem = HexDump(buf);

                        MEMORY_BASIC_INFORMATION infox;
                        int resultx = VirtualQueryEx(phx, ((IntPtr)Convert.ToUInt64(etw_event_string.Split(':')[4])), out infox, Marshal.SizeOf<MEMORY_BASIC_INFORMATION>());
                        if (resultx != 0)
                            listView1.BeginInvoke((MethodInvoker)delegate
                            {

                                DetectDesc = "EKKO Technique Detected (10 times)";

                                iList5 = new ListViewItem();
                                iList5.Name = DetectDesc + " In Process: " + _ProcessName + " With PID: " + __PID
                                     + "\nDetails: [TID: " + tid + "] with BaseAddress: "
                                     + etw_event_string.Split(':')[7] + "," + etw_event_string.Split(':')[8]
                                     + "\n\n[ETW VirtualMemAlloc Event] => " + etw_event_string.ToString() + "\n\n"
                                     + "EKKO StartAddr: [" + infox.BaseAddress.ToString("X8")
                                     + "] Protection Mode: [0x04 changed to 0x20" + "]\n\n"
                                     + "[!] Found " + "1" + " of 11"
                                     + "\nMemory Bytes: " + "\n"
                                     + dumpmem;
                                iList5.SubItems.Add(etw_event_string.Split(']')[0].Split('[')[1]);
                                iList5.SubItems.Add(_ProcessName + ":" + __PID);
                                iList5.SubItems.Add(tid);
                                iList5.SubItems.Add("[!] Found " + "1" + " of 11");
                                iList5.SubItems.Add("VirtualMemAlloc");
                                iList5.SubItems.Add(etw_event_string);
                                iList5.SubItems.Add(DetectDesc + " In Process: " + _ProcessName + " With PID: " + __PID
                                     + "\nDetails: [TID: " + tid + "] with BaseAddress: "
                                     + etw_event_string.Split(':')[7] + "," + etw_event_string.Split(':')[8]);
                    ...
                    if (RW_count > 60 && RX_count > 60) { break; }
