...
#### warning: `VirtualMemAllocMon.exe has ETW C# codes (https://www.nuget.org/packages/Microsoft.Diagnostics.Tracing.TraceEvent) for monitoring ETW Events and this code has Confilict with some tools like TCPView (Sysinternals), or maybe i should say TCPView has confilict with ETW, so if you use these tools (like TCPView/Process Explorer...) with VirtualMemAllocMon.exe at same time then VirtualMemAllocMon.exe code will be crash and VirtualMemAllocMon.exe Memory scanner will not work for SysPM2Monitor2_7.exe .... , (i will work on this thing to fix it but for sure dont use these tools (TCPView,...) at same time with SysPM2Monitor2_7.exe)`
...
md5 info:
       ...
       84bd6cd4f7-8bf61d7dbd-59ccfc9e9882 => SysPM2Monitor2_7.exe [v2.7.17.59]
